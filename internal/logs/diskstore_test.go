...
// TestStoreStatsWithMissingChunksDirectory pins the collector-error policy
// from ARCHITECTURE_NOTES.md against the specific facts of this store:
// NewStore creates chunksDir unconditionally (MkdirAllSync), so a missing
// chunks directory at Stats time cannot mean "not flushed yet" -- it means
// something deleted the directory out from under the store, which is an
// operational failure. Reporting that as a healthy streams=2/chunks=0/bytes=0
// with a nil error (the old behavior) would show a confident zero on the
// dashboard instead of a gap, and obs_collector_errors_total would never
// move. Stats must instead return an error so the collector emits a gap and
// counts it.
func TestStoreStatsWithMissingChunksDirectory(t *testing.T) {
	...
	// Stats must surface this as an error, never a healthy zero.
	if _, _, _, err := s.Stats(); err == nil {
		t.Fatal("Stats with missing chunks dir: got nil error, want an error so the collector emits a gap instead of a confident zero")
	}
}
