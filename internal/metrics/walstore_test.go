...
func TestLabelIndex_IngestFlushRestartQuery(t *testing.T) {
	...
	got := mustSeries(t, e, metrics.MetadataFilter{Selectors: []metrics.Selector{{MetricName: "http", Matchers: []metrics.Matcher{{Name: "job", Value: "api"}}}}})
	if len(got) != 1 {
		t.Fatalf("after restart Series matched %d, want 1", len(got))
	}
	if names := mustLabelNames(t, e, metrics.MetadataFilter{}); len(names) != 2 { // __name__, job
