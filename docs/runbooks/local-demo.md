...
| `make smoke-compose` exits with `already has containers or volumes` | only reachable when you set `OBS_COMPOSE_PROJECT` yourself at a project that is already in use; the test will not run `down -v` on a stack it did not create, because that deletes named volumes | remove it yourself, pick another `OBS_COMPOSE_PROJECT`, or re-run with `OBS_COMPOSE_REPLACE_STACK=1` |
| `make smoke-compose` exits with `another run of this test already holds compose project` | two runs were pointed at the same `OBS_COMPOSE_PROJECT`; they cannot tell whose stack is whose | wait for the other run, or give this one a project of its own with `OBS_COMPOSE_PROJECT=<name>` |
