...
- [x] Verify: a fresh-clone pass following only the README reaches a working demo without guessing — **closed: walked `docs/runbooks/local-demo.md` top to bottom from a fresh clone at `5ca771f` on 2026-09-24, with Docker available.** (1) every documented `localhost` URL answered, including the logs-dashboard curl; (2) the durability block printed `durable: marker 843329699 survived a real restart` and exited `0`; (3) `make smoke-compose` ran green (`71 passed, 0 failed`, exit `0`) and all four troubleshooting-table messages are still present in `tests/e2e/compose_smoke.sh`. `make smoke` also passed in full, `13/13` then `33/33`. A first pass at `36a50dc` caught the drift this gate was made for: the logs curl queried `service="sample-app"`, a value the sample app's log streams never carry — they use `service` ∈ {`api`, `worker`}, and `sample-app` is only the sample app's *metrics* label (Phase 5.1) — fixed in `5ca771f` before this pass closed the gate
