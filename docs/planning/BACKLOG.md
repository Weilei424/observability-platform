...
- [x] Verify *(requires Docker)*: `make smoke-compose` exits 0, including the volume-panel assertions — run in CI rather than locally, and re-run on every push to `main`, so cite the most recent green one: the `Compose Stack E2E (through Grafana)` job at `461320b` ([run 31155052122](https://github.com/Weilei424/observability-platform/actions/runs/31155052122)). That job is what exercises the panel expression, Explore's `| drop __error__` form, and the parsed multi-series assertions against a real Grafana on the real stack
