...
(or `make bench-k6`), which builds + starts a throwaway backend, seeds it, runs
every scenario, and tears down. Run a single script standalone like this:
...
|---|---|---|
| `ingest.js` | ingest req/s, samples/s, p50/p95/p99; also seeds data | `POST /api/v1/ingest/metrics` |
